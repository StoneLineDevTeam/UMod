...
void AUModHUD::DrawHUD()
{
	...
	if (localPlyCL == NULL || localPlyCL->PlayerState == NULL || ctrl == NULL) {
		DrawSimpleText(FString("THIS CLIENT IS ERRORED"), 16, 16, 2, FColor(255, 0, 0), TEXT_ALIGN_LEFT);
	} else {
		FString txt = localPlyCL->PlayerState->PlayerName;
		DrawSimpleText(FString("Playing as ") + txt, 16, 16, 2, FColor(255, 0, 0), TEXT_ALIGN_LEFT);
	}

	//Health and Armor Box

	//Yeah again localPlyCL is null thanks to buggy engine !!
